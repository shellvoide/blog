...
**Classification.** CWE-476 (NULL Pointer Dereference), an incomplete fix of [GHSA-p6wj-qrr4-pgh5](https://github.com/advisories/GHSA-p6wj-qrr4-pgh5). `CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:H` = 7.5 (High), the rating the maintainers accepted: an unrecovered panic is a full-process crash on non-`net/http` hosts and in goroutine-spawned validation. A plain `net/http` server bounds it to a per-request crash, which lowers the blast radius, not the class. Affected from v0.136.0 through v0.144.0.
