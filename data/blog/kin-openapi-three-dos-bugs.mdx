...
| 2   | nil-`items` panic in `openapi3filter`              | A prior fix guarded three decoder paths; three more reach the same sink        | Medium to High (5.3 to 7.5) |
| 3   | YAML `NaN`/`Inf` panic on OAS 3.1                  | The built-in visitor rejects `NaN`/`Inf`; the 3.1 dispatch hands it straight through | Medium to High (5.3 to 7.5) |
...
<BarChart
  title="CVSS base score behind a standard net/http server"
  max={10}
  data={[
    { label: 'Circular recursion', value: 7.5, display: '7.5', highlight: true },
    { label: 'nil-items panic', value: 5.3, display: '5.3' },
    { label: 'YAML NaN/Inf', value: 5.3, display: '5.3' },
  ]}
  caption="In the most common deployment, net/http recovers the two panics per connection, so they land at Medium. Only the recursion crash, a fatal runtime error, stays High here, because no per-connection recovery can catch it."
/>
...
<ReasoningTrace
  ...
  lines={[
    ...
    },
    {
      time: '02:13',
      role: 'AGENT',
      text: `Important distinction for severity: unbounded recursion in Go is not a panic, it is fatal error: stack overflow. recover() cannot catch it. net/http wraps each connection in a recover, but that does not help here, so the whole process dies, taking every other in-flight request with it. This is A:H regardless of host, unlike a normal nil-pointer panic.`,
    },
  ]}
/>

That second line is the whole severity argument. A normal Go panic is per-goroutine and recoverable, so behind a standard `net/http` server it only kills the one connection. A stack overflow is a fatal runtime error: `recover()` does not catch it, the per-connection recovery `net/http` installs does nothing, and the entire process exits. One request takes down every unrelated client on the same instance, and no amount of handler-level defensiveness brings them back.

To prove it end to end, KLUE built a legal OAS 3.0 spec with a self-referential `allOf` and served it behind an ordinary `net/http` handler:
...
Sending GET /test?data=x through a real net/http server...
...
**Classification.** CWE-476 (NULL Pointer Dereference), an incomplete fix of [GHSA-p6wj-qrr4-pgh5](https://github.com/advisories/GHSA-p6wj-qrr4-pgh5). `CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:L` = 5.3 (Medium) behind `net/http` recovery, rising to 7.5 (High) with `A:H` where that recovery does not run. Affected from v0.136.0 through v0.144.0.
...
**Classification.** CWE-754 (Improper Check for Unusual or Exceptional Conditions). `CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:L` = 5.3 (Medium) behind `net/http`, 7.5 (High) with `A:H` on hosts without that recovery. Reachable on any OAS 3.1 endpoint that accepts a YAML body carrying a numeric constraint.

## The discipline to rate them honestly

Three denial-of-service bugs is an easy thing to over-sell, so it is worth being precise about how much each one actually costs, because they are not equal.

Two of the three, the nil-`items` panic and the YAML `NaN` panic, are ordinary Go panics. Behind a standard `net/http` server, `net/http` recovers them per connection: the client sees a dropped request, and the process keeps serving everyone else. That is a per-request denial of service, real but bounded, and we rate it `A:L` (CVSS 5.3) for that deployment. The full-process rating, `A:H` (CVSS 7.5), applies only where that recovery does not run, which is a genuine set of deployments (non-`net/http` hosts, or validation performed in an application-spawned goroutine) but not the common one. We reproduced both configurations rather than assume the worse one.

The circular-recursion bug is the exception, and that is the whole reason it sits at the top of the table. A stack overflow is a fatal runtime error, not a recoverable panic, so `net/http`'s per-connection recovery does nothing and the process dies regardless of host. There is no bounded, per-request version of it. We verified that directly: the same PoC that a `net/http` server would have shrugged off for the other two took the server down for this one. `A:H` everywhere, CVSS 7.5, and it earns the rating in a way the other two only do conditionally.

<BarChart
  title="How each score moves with the host"
  max={10}
  data={[
    { label: 'Recursion, any host', value: 7.5, display: '7.5', highlight: true },
    { label: 'nil-items, net/http', value: 5.3, display: '5.3' },
    { label: 'nil-items, other host', value: 7.5, display: '7.5' },
    { label: 'NaN/Inf, net/http', value: 5.3, display: '5.3' },
    { label: 'NaN/Inf, other host', value: 7.5, display: '7.5' },
  ]}
  caption="The two recoverable panics swing from 5.3 to 7.5 depending on whether net/http per-connection recovery sits in the path. The recursion crash does not swing: it is 7.5 everywhere."
/>

Naming that difference out loud is the point. Inflating the two recoverable panics to "full-process crash" full stop would have been the easy way to make the report look scarier, and it would have been wrong, and a maintainer would have caught it and trusted the next submission less.
...
All three findings were reported to the kin-openapi maintainers with full proof-of-concept code, the exact unguarded call sites, and per-deployment severity that separates the recoverable panics from the unrecoverable stack overflow. Each was reviewed and accepted: the circular-recursion crash, the incomplete-fix nil-`items` panic, and the OAS 3.1 `NaN`/`Inf` panic. No CVE identifiers had been assigned at the time of writing; we will update this post with them if and when they issue. Our thanks to the maintainers for a fast, straightforward triage, and for a codebase whose existing guards made the fixes obvious once the gaps were named.
